...
RELIABLE_EDT_COLS = [
    "EDT_125Hz", "EDT_500Hz", "EDT_1kHz", "EDT_2kHz", "EDT_4kHz",
]
...
# Default "reliable" label set for NN distillation (EDT mid/high + best C80 bands)
RELIABLE_OUTPUT_COLS = RELIABLE_EDT_COLS + RELIABLE_C80_COLS
...
def generate_training_data_from_gp(
    ...
    Generate synthetic training data for a downstream NN (e.g. Grasshopper/Pug):
      ...
      - Use the trained GP teacher to predict labels Y (selected outputs)
    ...
    pd.DataFrame(X_gen, columns=IN_COLS).to_csv(x_csv, index=False)
        # y_col_names length should match number of trained GPs
    if len(y_col_names) != n_out:
        raise ValueError(f"y_col_names has length {len(y_col_names)} but model has {n_out} outputs")
    pd.DataFrame(Y_gen, columns=y_col_names).to_csv(y_csv, index=False)
...
def main():
    ...
    ap.add_argument(
    "--command",
    choices=["train", "generate_training_data", "generate_training_data_reliable"],
    default="train",
    help="train: run full GP training + maps/suggestions. "
         "generate_training_data: fit GP teachers for all EDT bands and export X/Y datasets for NN training. "
         "generate_training_data_reliable: fit GP teachers only for the most reliable labels (EDT mid/high + C80_1kHz/4kHz) and export X/Y datasets for NN training.",)
    ap.add_argument("--n_training", type=int, default=50_000,
                help="Number of (X,Y) samples to generate when --command starts with generate_training_data.")
    ap.add_argument("--gen_seed", type=int, default=0,
                help="RNG seed used when sampling points for --command starts with generate_training_data.")
    ...
    # --- Command(s): generate NN training data (X,Y) from the GP teacher ---
    if args.command in ("generate_training_data", "generate_training_data_reliable"):
        if args.command == "generate_training_data":
            y_cols = EDT_COLS
        else:
            y_cols = list(RELIABLE_EDT_COLS)
            if has_clarity:
                y_cols += list(RELIABLE_C80_COLS)
            else:
                print("\nWARNING: clarity columns not found in output file. Proceeding with reliable EDT only.")

        # Select targets (and apply the same wall-margin mask as the GP training)
        Y_targets = df_out[y_cols].to_numpy(dtype=float)[keep]

        # Fit GPs on the full (filtered) dataset for the selected outputs. (Means are unaffected by sigma calibration.)
        x_scaler = StandardScaler().fit(X)
        y_scaler = StandardScaler().fit(Y_targets)
        Xs = x_scaler.transform(X)
        Ys = y_scaler.transform(Y_targets)

        print(f"\nTraining GP teacher (for NN data generation) on {len(y_cols)} outputs: {', '.join(y_cols)}")
        gps = train_gps(Xs, Ys, kernel, n_restarts=args.n_restarts, suppress_warnings=args.suppress_warnings)

        sigma_scale = np.ones(Y_targets.shape[1], dtype=float)
        ...
        # Lightweight metadata for downstream NN training
        meta_path = os.path.join(args.outdir, "nn_dataset_meta.txt")
        with open(meta_path, "w", encoding="utf-8") as f:
            f.write(f"command={args.command}\n")
            f.write(f"n_training={args.n_training}\n")
            f.write(f"wall_margin={args.wall_margin}\n")
            f.write(f"emitter_xy={emitter_xy}\n")
            f.write(f"receiver_xy={receiver_xy}\n")
            f.write(f"y_cols={y_cols}\n")
            f.write("angle_rule=radial_from_emitter\n")
        print(f"Saved NN dataset metadata:  {meta_path}")
        return
