...
def evaluate_holdout_surrogate(
    ...
    n_total = X.shape[0]
    if n_total < 10:
        print("\nHold-out eval skipped: too few points.")
        return
    ...
    if holdout_n >= n_total:
        print("\nHold-out eval skipped: holdout_n >= n_total.")
        return

    tr_idx, ho_idx = _pick_holdout_indices(n_total=n_total, holdout_n=holdout_n, seed=seed)

    Xtr, Ytr, xytr = X[tr_idx], Y[tr_idx], train_xy[tr_idx]
    Xho, Yho, xyho = X[ho_idx], Y[ho_idx], train_xy[ho_idx]
    ...
    mpath = os.path.join(outdir, "holdout_metrics.csv")
    ...
    ppath = os.path.join(outdir, "holdout_predictions.csv")
    ...
    fig.tight_layout()
    out_png = os.path.join(outdir, "holdout_parity_all.png")
