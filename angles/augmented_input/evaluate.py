...
- Loads scalers, test split, and either:
    * all ensemble members model_m*.keras (averaged), or
    * model.keras (fallback) or best_model.keras (if present)
- Produces:
...
- Appends metrics to metrics.json and saves plots to --datadir
"""
...
def main():
    ...
    # Feature names (for residual plots)
    ...
    if member_paths:
        ...
        # Fallback: explicit (--model) or model.keras / best_model.keras
        if args.model.strip():
